 ...
 *
 *  - Earned floor, raised by deterministic detections. Halves after each full day without a new
 *    one, so an old false alarm stops lowering the bar. An admin confirm pins it high instead.
...
class SuspicionManager(@Volatile var sensitivity: Int = 50) {
    ...
    private class Score {
        ...
        @Volatile var lastFloorBump: Long = 0
    }
    ...
    companion object {
        ...
        const val FLOOR_HALF_LIFE_MS = 86_400_000L
        const val MAX = 100.0
    }

    fun suspicion(player: UUID): Double {
        val s = scores[player] ?: return 0.0
        val floor = if (s.confirmed) maxOf(s.floor, CONFIRM_FLOOR) else s.floor
        return (floor + s.heat).coerceIn(0.0, MAX)
    }

    fun floor(player: UUID): Double = scores[player]?.floor ?: 0.0

    fun bumpFloor(player: UUID, amount: Double = DETERMINISTIC_FLOOR_BUMP, now: Long = System.currentTimeMillis()) {
    ...
    fun decay(now: Long = System.currentTimeMillis()) {
        scores.forEach { (_, s) ->
            if (now - s.lastSignal >= IDLE_BEFORE_DECAY_MS && s.heat > 0) {
                s.heat = (s.heat - HEAT_DECAY_PER_TICK).coerceAtLeast(0.0)
            }
            if (s.floor > 0 && now - s.lastFloorBump >= FLOOR_HALF_LIFE_MS) {
                s.floor = if (s.floor < 1.0) 0.0 else s.floor / 2
                s.lastFloorBump = now
            }
        }
        scores.entries.removeIf { !it.value.confirmed && it.value.floor == 0.0 && it.value.heat == 0.0 }
    }

    fun confirm(player: UUID) {
        scores.computeIfAbsent(player) { Score() }.confirmed = true
    }
