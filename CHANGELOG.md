...
### Fixed
- **A very fast rejoin can no longer count a new player's inventory twice.** The first time a player joins, the plugin writes down what they are carrying as their starting point. It checked whether that had been done, then counted the inventory, then wrote it down, so two joins close together could both find nothing yet and both add the full inventory. That left the player with room to dupe into. Only one starting point can now be in progress per player, and the "already done" mark is written before the inventory is counted.
